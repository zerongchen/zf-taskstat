...
# wlan月粒度表
to_job_ubas_wlan_m=job_ubas_wlan_m
...
OUTPUTPATH=/user/hive/warehouse/${hive_database}.db/${to_job_ubas_wlan_m}/${SDATE}

stat_sql=" INSERT OVERWRITE TABLE ${to_job_ubas_wlan_m} partition(dt='${SDATE}')
...
hive -e   "use ${hive_database};
         alter table ${to_job_ubas_wlan_m} add if not exists partition(dt='${SDATE}') location '${OUTPUTPATH}' ;
		  set mapred.job.name=to_job_ubas_wlan_m_${SDATE};
         ${stat_sql}"
ret=$?
if [ $ret -ne 0 ]
then 
...
if [ "${IMPALA_SERVER}" = "" ];then
	impala-shell -q "refresh  ${hive_database}.${to_job_ubas_wlan_m} "
else
	impala-shell -q "refresh  ${hive_database}.${to_job_ubas_wlan_m} " -i ${IMPALA_SERVER}
fi
exit 0
