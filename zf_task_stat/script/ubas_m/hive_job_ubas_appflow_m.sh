...
#通用流量小时粒度表
to_job_ubas_appflow_m=job_ubas_appflow_m
...
OUTPUTPATH=/user/hive/warehouse/${hive_database}.db/${to_job_ubas_appflow_m}/${SDATE}

stat_sql=" INSERT OVERWRITE TABLE ${to_job_ubas_appflow_m} partition(dt='${SDATE}')
...
hive -e   "use ${hive_database};
         alter table ${to_job_ubas_appflow_m} add if not exists partition(dt='${SDATE}') location '${OUTPUTPATH}' ;
		  set mapred.job.name=to_job_ubas_appflow_m_${SDATE};
         ${stat_sql}"
ret=$?
if [ $ret -ne 0 ]
then 
