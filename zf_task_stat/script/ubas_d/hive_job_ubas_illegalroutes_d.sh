...
#--------------------------------------------------
# 非法路由分钟粒度表
from_job_ubas_illegalroutes=job_ubas_illegalroutes
# 非法路由小时粒度表
to_job_ubas_illegalroutes_d=job_ubas_illegalroutes_d
...
OUTPUTPATH=/user/hive/warehouse/${hive_database}.db/${to_job_ubas_illegalroutes_d}/${DATE}

# nodeip: 非法接入点位置
stat_sql=" INSERT OVERWRITE TABLE ${to_job_ubas_illegalroutes_d} partition(dt='${DATE}')
       select '${DATE}' as stat_time
              ...
              ,probe_type
				,area_id
            from ${from_job_ubas_illegalroutes} where dt='${DATE}'
			 group by nodeip,cp,probe_type,area_id"

echo "stat_sql=${stat_sql}"

beeline_sql="use ${hive_database};
         alter table ${to_job_ubas_illegalroutes_d} add if not exists partition(dt='${DATE}') location '${OUTPUTPATH}' ;
		  set mapred.job.name=to_job_ubas_illegalroutes_d_${DATE};
...
if [ "${IMPALA_SERVER}" = "" ];then
	impala-shell -q "refresh  ${hive_database}.${to_job_ubas_illegalroutes_d} "
else
	impala-shell -q "refresh  ${hive_database}.${to_job_ubas_illegalroutes_d} " -i ${IMPALA_SERVER}
fi
