...
#--------------------------------------------------
# web流量分钟粒度表
from_job_ubas_webflow_h=job_ubas_webflow_h
# web流量小时粒度表
to_job_ubas_webflow_d=job_ubas_webflow_d
...
OUTPUTPATH=/user/hive/warehouse/${hive_database}.db/${to_job_ubas_webflow_d}/${DATE}

# select lpad(conv(13,10,2),32,'0') ;
stat_sql=" INSERT OVERWRITE TABLE ${to_job_ubas_webflow_d} partition(dt='${DATE}')
       select '${DATE}' as stat_time
              ...
              ,probe_type
				,area_id
            from ${from_job_ubas_webflow_h} where dt='${DATE}'
...
beeline_sql="use ${hive_database};
         alter table ${to_job_ubas_webflow_d} add if not exists partition(dt='${DATE}') location '${OUTPUTPATH}' ;
		  set mapred.job.name=to_job_ubas_webflow_d_${DATE};
...
if [ "${IMPALA_SERVER}" = "" ];then
	impala-shell -q "refresh  ${hive_database}.${to_job_ubas_webflow_d} "
else
	impala-shell -q "refresh  ${hive_database}.${to_job_ubas_webflow_d} " -i ${IMPALA_SERVER}
fi
