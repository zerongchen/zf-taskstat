...
from_job_traffic=job_ubas_traffic
to_job_socket_save=job_ud_monitor_traffic_savedata_h
#流量流向分析结果上报
PACKETTYPE=1
PACKETSUBTYPE=196
...
hive -e "use ${DATABASE_SCHEMA};
        set mapred.job.name=job_traffic_socket_save_${DATE}_${HOURS}_PACKETSUBTYPE_${PACKETSUBTYPE};
		insert overwrite directory '${OUTPUTPATH}' ${stat_sql};"

ret=$?
if [ $ret -ne 0 ]
then
        echo `date +"%Y-%m-%d %H:%M:%S"`   "INSERT overwrite error [$DATE][$HOURS],exit $ret !"
        exit $ret
fi
