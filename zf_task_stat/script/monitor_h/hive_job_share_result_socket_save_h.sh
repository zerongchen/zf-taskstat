...
from_job_table=job_ubas_share_result
to_job_socket_save=job_ud_monitor_share_result_savedata_h
#一拖N用户行为分析（检测结果）
PACKETTYPE=1
PACKETSUBTYPE=130
...
hive -e " use ${DATABASE_SCHEMA};
        set mapred.job.name=job_share_result_socket_save_${DATE}_${HOURS};
