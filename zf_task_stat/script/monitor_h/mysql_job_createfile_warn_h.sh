               ...
               SELECT
                 ...
                 SUM(
                   CASE
                     ...
                     THEN 1
                     WHEN  u.fileupload_time-t.file_time >= ${UPLOAD_TIMEOUT}
                     THEN 1
                     ELSE 0
                   END
                 ) AS warn_file_num
               FROM
                 ${createfile_detail_table} t
                 LEFT JOIN ${uploadfile_detail_table} u
                   ON t.file_name = u.file_name
                   AND t.file_type = u.file_type
                 WHERE t.file_time >= ${lastTimestamp}
                 and t.file_time < ${currTimestamp}
               GROUP BY t.file_type
            ) A WHERE warn_file_num > 0
           "

echo "createFile collectSql=${collectSql}"
...
if [ $ret -ne 0 ]
then
        echo `date +"%Y-%m-%d %H:%M:%S"`   "send create file error msg error,exit $ret !"
        exit $ret
fi
...
java ${CLASS_PATH} ${MAIN_CLASS} 1 ${param}
ret=$?
if [ $ret -ne 0 ]
then
