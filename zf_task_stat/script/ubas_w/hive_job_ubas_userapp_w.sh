...
# 指定用户应用周粒度表
to_job_ubas_userapp_w=job_ubas_userapp_w
...
OUTPUTPATH=/user/hive/warehouse/${hive_database}.db/${to_job_ubas_userapp_w}/${SDATE}

stat_sql=" INSERT OVERWRITE TABLE ${to_job_ubas_userapp_w} partition(dt='${SDATE}')
...
hive -e   "use ${hive_database};
         alter table ${to_job_ubas_userapp_w} add if not exists partition(dt='${SDATE}') location '${OUTPUTPATH}' ;
		  set mapred.job.name=to_job_ubas_userapp_w_${SDATE};
         ${stat_sql}"
ret=$?
if [ $ret -ne 0 ]
then 
...
if [ "${IMPALA_SERVER}" = "" ];then
	impala-shell -q "refresh  ${hive_database}.${to_job_ubas_userapp_w} "
else
	impala-shell -q "refresh  ${hive_database}.${to_job_ubas_userapp_w} " -i ${IMPALA_SERVER}
fi
exit 0
