...
#非法路由周粒度表
to_job_ubas_illegalroutes_w=job_ubas_illegalroutes_w

echo `date +"%Y-%m-%d %H:%M:%S"`  "--------------------------------------"
echo `date +"%Y-%m-%d %H:%M:%S"`  "---- zf-statmange 4.6.1 --------------"
echo `date +"%Y-%m-%d %H:%M:%S"`  "Shell Version 4.6.1 update(2018-06-27)"
echo `date +"%Y-%m-%d %H:%M:%S"`  "--------------------------------------"

OUTPUTPATH=/user/hive/warehouse/${hive_database}.db/${to_job_ubas_illegalroutes_w}/${SDATE}

stat_sql=" INSERT OVERWRITE TABLE ${to_job_ubas_illegalroutes_w} partition(dt='${SDATE}')
...
hive -e   "use ${hive_database};
         alter table ${to_job_ubas_illegalroutes_w} add if not exists partition(dt='${SDATE}') location '${OUTPUTPATH}' ;
		  set mapred.job.name=to_job_ubas_illegalroutes_w_${SDATE};
         ${stat_sql}"
ret=$?
if [ $ret -ne 0 ]
then 
...
if [ "${IMPALA_SERVER}" = "" ];then
	impala-shell -q "refresh  ${hive_database}.${to_job_ubas_illegalroutes_w} "
else
	impala-shell -q "refresh  ${hive_database}.${to_job_ubas_illegalroutes_w} " -i ${IMPALA_SERVER}
fi
exit 0
