...
#--------------------------------------------------
#通用流量分钟粒度表
from_job_ubas_appflow_d=job_ubas_appflow_d
#通用流量小时粒度表
to_job_ubas_appflow_w=job_ubas_appflow_w
...
OUTPUTPATH=/user/hive/warehouse/${hive_database}.db/${to_job_ubas_appflow_w}/${SDATE}

stat_sql=" INSERT OVERWRITE TABLE ${to_job_ubas_appflow_w} partition(dt='${SDATE}')
      select '${SDATE}' as stat_time
              ...
              ,probe_type
				,area_id
            from ${from_job_ubas_appflow_d} where dt>='${SDATE}' and dt<'${EDATE}'
...
hive -e   "use ${hive_database};
         alter table ${to_job_ubas_appflow_w} add if not exists partition(dt='${SDATE}') location '${OUTPUTPATH}' ;
		  set mapred.job.name=to_job_ubas_appflow_w_${SDATE};
         ${stat_sql}"
ret=$?
if [ $ret -ne 0 ]
then 
